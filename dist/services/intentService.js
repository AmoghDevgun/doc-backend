export async function detectIntent(userText) {
    ...
    const match = (patterns) => patterns.some(p => text.includes(p));
    // =============================
    // 1️⃣ SYMPTOM ANALYSIS
    // =============================
    if (match(["i have", "feeling", "suffering", "pain", "cough", "fever", "ache", "symptom", "vomit", "tired", "dizzy", "sick"]) ||
    ...
    // =============================
    if (match(["book", "schedule", "make appointment", "set appointment", "reserve appointment"]) ||
    ...
    // =============================
    if (match(["my appointments", "show appointments", "check appointments", "list appointments", "view my schedule"]) ||
